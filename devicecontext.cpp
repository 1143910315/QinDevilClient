...
bool DeviceContext::GetDeviceContext(void *hwnd) {
    if (handleDeviceContext != 0) {
        ReleaseDC((HWND)hwnd, (HDC)handleDeviceContext);
    }
    handleWindow = hwnd;
    handleDeviceContext = GetDC((HWND)handleWindow);
    if (handleDeviceContext == 0) {
...
bool DeviceContext::CacheRegion(const QRect &rect) {
    ...
    handleCompatibleDeviceContext = CreateCompatibleDC((HDC)handleDeviceContext);
